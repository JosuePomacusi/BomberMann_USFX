...
void LevelScene::spawnBomb(GameGraphicObject* object)
{
 ...
 int bangSpawnCellsX = 5;
//
//int bangSpawnPositionsX[bangSpawnCellsX][2] = { {0, 0},
//                                                   {0, 1},
//                                                   {1, 0},
//                                                   {0, -1},
//                                                   {-1, 0} };
void LevelScene::spawnBang(GameGraphicObject* object)
{
    ...
    for (unsigned int i = 0; i < bangSpawnCellsX+bombas ; i++)
    {
        ...
        bang->setPosition(object->getPositionX() + bombas+bangSpawnPositions[i][0] * scaledTileSize,
            object->getPositionY() + bombas+bangSpawnPositions[i][1] * scaledTileSize);
        addObject(bang);
...
void LevelScene::spawnItemBoombaTamaño(GameGraphicObject* objectItem)
{
    // create door in position
    doorItemBomba =
        std::make_shared<Sprite>(gameManager->getAssetManager()->getTexture(GameTexture::Bomba), gameManager->getRenderer());
    doorItemBomba->setSize(scaledTileSize, scaledTileSize);
    doorItemBomba->setPosition(objectItem->getPositionX(), objectItem->getPositionY());
    insertObject(doorItemBomba, backgroundObjectLastNumber);
}
...
void LevelScene::updatePlayerCollision()
{
    ...
    if (doorItemBomba != nullptr)
    {
        if (isCollisionDetected(playerRect, doorItemBomba->getRect()))
        {
            removeObject(doorItemBomba);
...
void LevelScene::destroyBrickItem(std::shared_ptr<GameGraphicObject> brick)
{
    // we need door if don't have
    if (doorItemBomba == nullptr)
    {
        ...
        // random for door spawn
        const auto seed = std::chrono::high_resolution_clock::now().time_since_epoch().count();
        auto randDoorItem = std::bind(std::uniform_int_distribution<int>(0, doorSpawnRandomize),
            std::mt19937(static_cast<unsigned int>(seed)));
        // spawn door if we can
        if (randDoorItem() == 0 || bricksCount <= 1)
        {
            spawnItemBoombaTamaño(brick.get());
            //  brickSpawnRandomize
